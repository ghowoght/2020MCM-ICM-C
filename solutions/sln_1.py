...
#pd.DataFrame(df.review_length).boxplot()
#plt.show()
...
X_train,X_test,Y_train,Y_test = train_test_split(df.ix[:,:6],df.helpful_votes,train_size=.80)
...
#Y = model.predict(df.ix[:,:6])
#plt.plot(range(len(Y)),Y,'b',label="predict")
#plt.plot(range(len(Y)),df.helpful_votes,'r',label="test")
#plt.show()
